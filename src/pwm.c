...
void config_PWM(TIM_GPIO_t pwm_pin, uint32_t prescaler, uint32_t period) {
	...
	TIM_TypeDef* tim = id_to_dev(dev.dev_id);
	uint8_t channel = dev.dev_id.sub & 0x3UL;
	fconfig_GPIO(
			int_to_GPIO(dev.port_num), dev.pin_num, GPIO_alt_func,
			GPIO_no_pull, GPIO_push_pull, GPIO_very_high_speed, dev.alt_func
	);
